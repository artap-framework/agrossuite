...
namespace Step26
{
  ...
  void HeatEquation<dim>::setup_system()
  {
    ...
    */
    ...
    // assemble
    dealii::QGauss<2>  quadrature_formula(fe.degree+1);
    dealii::QGauss<2-1> face_quadrature_formula(fe.degree+1);
                ...
                for (unsigned int i = 0; i < dofs_per_cell; ++i)
                    for (unsigned int j = 0; j < dofs_per_cell; ++j)
                        for (unsigned int q_point = 0; q_point < n_face_q_points; ++q_point)
                            cell_matrix(i, j) += ALPHA * fe_face_values.shape_value(i, q_point) * fe_face_values.shape_value(j, q_point) * fe_face_values.JxW(q_point);

                for (unsigned int q_point = 0; q_point < n_face_q_points; ++q_point)
                    for (unsigned int j = 0; j < dofs_per_cell; ++j)
                        cell_rhs(j) += ALPHA * TEXT * fe_face_values.shape_value(j, q_point) * fe_face_values.JxW(q_point);
