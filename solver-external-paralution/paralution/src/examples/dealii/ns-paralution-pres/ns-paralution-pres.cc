//
// PARALUTION modification -- Dimitar Lukarski
//                            www.paralution.com 2013
//
...
#include <plug-ins/paralution_dealii.hpp>
...
namespace Step35
{
  ...
  class NavierStokesProjection
  {
    ...
    protected:
      ...
      paralution::LocalMatrix<double> paralution_pres_iterative;     
      paralution::LocalVector<double> paralution_pres_tmp, paralution_phi_n;
      ...
      paralution::MultiColoredILU<paralution::LocalMatrix<double>,
                                  paralution::LocalVector<double>,
                                  double> paralution_pres_precond;
  ...
  create_triangulation_and_dofs (const unsigned int n_refines)
  {
    ...
    paralution_pres_tmp.Allocate("pres_tmp", pres_tmp.size());
    paralution_phi_n.Allocate("phi_n", phi_n.size());
  ...
  NavierStokesProjection<dim>::projection_step (const bool reinit_prec)
  {
    ...
    std::cout << "DEAL II CG:" << (tack-tick)/1000000 << " sec" 
              << " iter=" << solvercontrol.last_step() << std::endl;
    ...
    */
    ...
    import_dealii_matrix(sparsity_pattern_pressure,
                         pres_iterative,
                         &paralution_pres_iterative);

    paralution_pres_iterative.ConvertToCSR();

    tack = paralution_time();
    std::cout << "Import matrix:" << (tack-tick)/1000000 << " sec" << std::endl;


    ls.Clear();
    ls.SetOperator(paralution_pres_iterative);
    ls.SetPreconditioner(paralution_pres_precond);

    /*
    // using rel
    ls.Init(1e-13, // abs
            vel_eps*pres_tmp.l2_norm(), // rel
            1e+8,
            100000); // vel_max_its);
    */

    // using abs
    ls.Init(vel_eps*pres_tmp.l2_norm(), // abs
            1e-13, // rel
            1e+8,
            100000); // vel_max_its);
    ...
    paralution_pres_iterative.ConvertToHYB();

    tick = paralution_time();

    import_dealii_vector(pres_tmp, &paralution_pres_tmp);
    import_dealii_vector(phi_n, &paralution_phi_n);

    tack = paralution_time();
    std::cout << "Import vectors:" << (tack-tick)/1000000 << " sec" << std::endl;

    tick = paralution_time();

    paralution_pres_iterative.MoveToAccelerator();
    paralution_phi_n.MoveToAccelerator();
    paralution_pres_tmp.MoveToAccelerator();
    ls.MoveToAccelerator();

    tack = paralution_time();
    std::cout << "Copy to GPU:" << (tack-tick)/1000000 << " sec" << std::endl;


    paralution_pres_iterative.info();
    
    tick = paralution_time();

    ls.Solve(paralution_pres_tmp, &paralution_phi_n);

    tack = paralution_time();
    std::cout << "Paralution CG:" << (tack-tick)/1000000 << " sec" << std::endl;

    tick = paralution_time();

    ls.MoveToHost();

    export_dealii_vector(paralution_phi_n, &phi_n);

    tack = paralution_time();
    std::cout << "Export vector:" << (tack-tick)/1000000 << " sec" << std::endl;
