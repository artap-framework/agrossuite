...
static PythonEngine *pythonEngine = nullptr;
...
PythonEngine::PythonEngine()
    : errorType(NULL), errorValue(NULL), errorTraceback(NULL), m_useProfiler(false),
      m_dictGlobal(NULL), m_functions(QString())
{}
...
void PythonEngine::init(int argc, char *argv[])
{
    ...
    addFunctions(readFileContent(datadir() + "/resources/pythonlab/functions_pythonlab.py"));
    ...
    // custom modules
    PyObject *importPython = PyRun_String(QString("import sys; sys.path.insert(0, \"%1/resources/pythonlab\"); sys.path.insert(0, \"%1/resources/python\")").
                                          arg(datadir()).toLatin1().data(), Py_file_input, globalDict(), localDict());
...
ErrorResult PythonEngine::parseError(bool clear)
{
    ...
    if (errorType != NULL && (errorString = PyObject_Str(errorType)) != NULL && (PyUnicode_Check(errorString)))
    {
    ...
    if (errorValue != NULL && (errorString = PyObject_Str(errorValue)) != NULL && (PyUnicode_Check(errorString)))
    {
        Py_INCREF(errorString);
        text.append("\n");
        text.append(QString::fromWCharArray(PyUnicode_AsUnicode(errorString)));
...
QList<PythonVariable> PythonEngine::variableList()
{
    ...
    for (int i = 0; i < PyList_Size(keys); ++i)
    {
        ...
        if (var.type == "bool")
        {
            var.value = PyLong_AsLong(value) ? "True" : "False";
        }
        else if (var.type == "int")
        {
            var.value = (int) PyLong_AsLong(value);
        }
