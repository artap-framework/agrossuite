...
FileBrowser::FileBrowser(QWidget *parent) : QListWidget(parent)
{
    ...
    actCreateDirectory = new QAction(iconAwesome(fa::foldero), tr("&Create directory"), this);
    connect(actCreateDirectory, SIGNAL(triggered()), this, SLOT(createDir()));

    actCreateFile = new QAction(iconAwesome(fa::filetexto), tr("Create &file"), this);
...
void FileBrowser::setDir(const QString &path, bool force)
{
    ...
    foreach(QString item, entries)
    {
        ...
        {
            addItem(new QListWidgetItem(iconAwesome(fa::foldero), item));
        }
        else
        {
            if (fileInfo.suffix() == "py")
                addItem(new QListWidgetItem(iconAwesome(fa::file), item));
            else
                addItem(new QListWidgetItem(iconAwesome(fa::filetexto), item));
        }
    }
