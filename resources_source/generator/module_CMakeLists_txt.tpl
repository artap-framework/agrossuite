...
IF (AGROS_BUILD_PLUGIN_STATIC)
    ADD_DEFINITIONS(-DQT_STATIC)
    ADD_DEFINITIONS(-DQT_STATICPLUGIN)
    ADD_LIBRARY(${PROJECT_NAME} STATIC ${SOURCES} ${HEADERS})
ELSE (AGROS_BUILD_PLUGIN_STATIC)
    ADD_DEFINITIONS(-DQT_SHARED)
    ADD_DEFINITIONS(-DQT_DLL)
    ADD_LIBRARY(${PROJECT_NAME} SHARED ${SOURCES} ${HEADERS})
ENDIF (AGROS_BUILD_PLUGIN_STATIC)

SET(LIBRARIES_TO_LINK ${QT_LIBRARIES} ${OPENGL_LIBRARIES} ${PYTHON_MODLIBS} ${PYTHON_LIB} ${PLUGINS_AGROS_LIBRARY} ${PTHREAD_LIBRARY} ${MUMPS_LIBRARIES} ${DEAL_II_LIBRARIES} ${UMFPACK_LIBRARY} ${AMD_LIBRARY} ${SuiteSparse_config_LIBRARY} ${BLAS_LIBRARIES})
