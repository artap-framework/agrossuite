...
void {{CLASS}}SurfaceIntegral::localAssembleSystem(const typename dealii::DoFHandler<2>::active_cell_iterator &cell_int,
        ...
        for (unsigned int face = 0; face < dealii::GeometryInfo<2>::faces_per_cell; ++face)
        {
            if (cell_int->face(face)->user_index() - 1 == iFace)
            {
                ...
                if ((m_analysisType == {{ANALYSIS_TYPE}}) && (m_coordinateType == {{COORDINATE_TYPE}}))
                {
                    ...
                    copy_data.results[{{VARIABLE_HASH}}u] += res;
                }
                {{/VARIABLE_SOURCE}}
            }
        }
...
void {{CLASS}}SurfaceIntegral::copyLocalToGlobal(const IntegralCopyData &copy_data)
{
    ...
    if ((m_analysisType == {{ANALYSIS_TYPE}}) && (m_coordinateType == {{COORDINATE_TYPE}}))
    {
        m_values[QLatin1String("{{VARIABLE}}")] += copy_data.results[{{VARIABLE_HASH}}u];
    }
    {{/VARIABLE_SOURCE}}
}
