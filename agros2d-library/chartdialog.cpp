...
void SceneViewChart::refresh()
{
    ...
    if ((ChartMode) m_computation->setting()->value(ProblemSetting::View_ChartMode).toInt() == ChartMode_Geometry)
        plotGeometry();
    else if ((ChartMode) m_computation->setting()->value(ProblemSetting::View_ChartMode).toInt() == ChartMode_Time)
        plotTime();
...
QVector<double> SceneViewChart::horizontalAxisValues(ChartLine *chartLine)
{
    ...
    switch ((ChartAxisType) m_computation->setting()->value(ProblemSetting::View_ChartHorizontalAxis).toInt())
    {
...
void SceneViewChart::plotGeometry()
{
    ...
    // variable component
    PhysicFieldVariableComp physicFieldVariableComp = (PhysicFieldVariableComp) m_computation->setting()->value(ProblemSetting::View_ChartVariableComp).toInt();
    ...
    // values
    ChartLine chartLine(Point(m_computation->setting()->value(ProblemSetting::View_ChartStartX).toDouble(),
                              m_computation->setting()->value(ProblemSetting::View_ChartStartY).toDouble()),
                        Point(m_computation->setting()->value(ProblemSetting::View_ChartEndX).toDouble(),
                              m_computation->setting()->value(ProblemSetting::View_ChartEndY).toDouble()),
                        m_computation->setting()->value(ProblemSetting::View_ChartHorizontalAxisPoints).toInt());
    ...
    // reverse x axis
    if (m_computation->setting()->value(ProblemSetting::View_ChartHorizontalAxisReverse).toBool())
    {
    ...
    for (int step = 0; step < times.count(); step++)
    {
        foreach (Module::LocalVariable variable, m_computation->postDeal()->activeViewField()->localPointVariables(m_computation->config()->coordinateType()))
        {
            ...
            if (stepIsAvailable)
            {
                ...
                Point point(m_computation->setting()->value(ProblemSetting::View_ChartTimeX).toDouble(),
                            m_computation->setting()->value(ProblemSetting::View_ChartTimeY).toDouble());
                std::shared_ptr<LocalValue> localValue = m_computation->postDeal()->activeViewField()->plugin()->localValue(m_computation.data(),
                                                                                                                            m_computation->postDeal()->activeViewField(),
                                                                                                                            step,
                                                                                                                            adaptiveStep,
