...
{
    return Point3(m_particleTracing->computation()->setting()->value(PostprocessorSetting::ParticleCustomForceX).toDouble(),
                  m_particleTracing->computation()->setting()->value(PostprocessorSetting::ParticleCustomForceY).toDouble(),
                  m_particleTracing->computation()->setting()->value(PostprocessorSetting::ParticleCustomForceZ).toDouble());
}
...
Point3 ParticleTracingForceDrag::force(int particleIndex, const Point3 &position, const Point3 &velocity)
{
    ...
    if (velocityReal.magnitude() > 0.0)
        forceDrag = velocityReal.normalizePoint() *
                - 0.5 * m_particleTracing->computation()->setting()->value(PostprocessorSetting::ParticleDragDensity).toDouble()
                * velocityReal.magnitude() * velocityReal.magnitude()
                * m_particleTracing->computation()->setting()->value(PostprocessorSetting::ParticleDragCoefficient).toDouble()
                * m_particleTracing->computation()->setting()->value(PostprocessorSetting::ParticleDragReferenceArea).toDouble();
...
Point3 ParticleTracingForceFieldP2P::force(int particleIndex, const Point3 &position, const Point3 &velocity)
{
    ...
    if (m_particleTracing->computation()->setting()->value(PostprocessorSetting::ParticleP2PElectricForce).toBool() ||
            m_particleTracing->computation()->setting()->value(PostprocessorSetting::ParticleP2PMagneticForce).toBool())
    {
        for (int i = 0; i < m_particleTracing->positions().size(); i++)
        {
            ...
            if (distance > 0)
            {
                ...
                if (m_particleTracing->computation()->setting()->value(PostprocessorSetting::ParticleP2PMagneticForce).toBool())
                {
                    Point3 r0, v0;
                    if (m_particleTracing->computation()->config()->coordinateType() == CoordinateType_Planar)
                    {
