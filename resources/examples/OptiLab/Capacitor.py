...
if __name__ == '__main__':
    from variant import SensitivityAnalysis
    from variant.parameter import Parameters, ContinuousParameter
    from variant.functional import Functionals, Functional

    import pythonlab

    parameters = Parameters([ContinuousParameter('w1', 0.02, 0.04),
                             ContinuousParameter('w2', 0.02, 0.04),
                             ContinuousParameter('l', 0.03, 0.05)])

    functionals = Functionals([Functional('C')])
    sensitivity = SensitivityAnalysis(Capacitor, parameters, functionals)

    sensitivity.analyse(10)

    C = sensitivity.model_dictionary.variables()['C']
    pythonlab.chart(sensitivity.model_dictionary.parameters()['w1'], C, 'w1', 'C')
    pythonlab.chart(sensitivity.model_dictionary.parameters()['w2'], C, 'w2', 'C')
    pythonlab.chart(sensitivity.model_dictionary.parameters()['l'], C, 'l', 'C')

    sensitivity.model_dictionary.save(problem_file = 'Capacitor.py')